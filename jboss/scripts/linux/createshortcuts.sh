...
"$INSTALLDIR/installer/xdg/xdg-icon-resource" install --size 32 "$INSTALLDIR/scripts/images/pg-launchjboss.png"

# Fixup the scripts

chmod ugo+x "$INSTALLDIR/installer/jboss/"*.sh
chmod ugo+x "$INSTALLDIR/scripts/"*.sh

# Fixup the XDG files (don't just loop in case we have old entries we no longer want)
_fixup_file "$INSTALLDIR/scripts/xdg/pg-postgresql.directory"
_fixup_file "$INSTALLDIR/scripts/xdg/pg-launchjboss.desktop"
...
"$INSTALLDIR/installer/xdg/xdg-desktop-menu" install --mode system \
      ...
      "$INSTALLDIR/scripts/xdg/pg-launchjboss.desktop"  || _warn "Failed to create the jboss menu"
