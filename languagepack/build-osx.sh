...
_build_languagepack_osx() {
   ...
   install_path="${PG_LANGUAGEPACK_OSX}.build"
   ...
   rm -f ${PG_LANGUAGEPACK_OSX}.build/LanguagePack-$PG_VERSION_LANGUAGEPACK.tar.bz2
...
EOT-LANGUAGEPACK
    ...
    echo "Removing last successful staging directory ($PG_LANGUAGEPACK_OSX)"
    ssh $PG_SSH_OSX "rm -rf $PG_LANGUAGEPACK_OSX" || _die "Couldn't remove the last successful staging directory directory"
    ssh $PG_SSH_OSX "mkdir -p $PG_LANGUAGEPACK_OSX" || _die "Couldn't create the last successful staging directory"
    ssh $PG_SSH_OSX "chmod ugo+w $PG_LANGUAGEPACK_OSX" || _die "Couldn't set the permission on the last successful staging directory"

    echo "Copying the complete build to the successful staging directory"
    ssh $PG_SSH_OSX "cp -rp ${PG_LANGUAGEPACK_OSX}.build/* $PG_LANGUAGEPACK_OSX" || _die "Couldn't copy the existing staging directory"

    ssh $PG_SSH_OSX "echo PG_VERSION_LANGUAGEPACK=$PG_VERSION_LANGUAGEPACK > $PG_LANGUAGEPACK_OSX/versions-osx.sh" || _die "Failed to write languagepack version number into versions-osx.sh"
    ssh $PG_SSH_OSX "echo PG_BUILDNUM_LANGUAGEPACK=$PG_BUILDNUM_LANGUAGEPACK >> $PG_LANGUAGEPACK_OSX/versions-osx.sh" || _die "Failed to write languagepack build number into versions-osx.sh"
...
_postprocess_languagepack_osx() {
    ...
    ssh $PG_SSH_OSX "cd $PG_LANGUAGEPACK_OSX; rm -f LanguagePack-$PG_VERSION_LANGUAGEPACK.tar.bz2"
    ssh $PG_SSH_OSX "cd $PG_LANGUAGEPACK_OSX; tar -jcvf LanguagePack-$PG_VERSION_LANGUAGEPACK.tar.bz2 *" || _die "Failed to create an archive of languagepack build"
    ...
    mv $WD/output/edb-languagepack-$PG_VERSION_LANGUAGEPACK-$PG_BUILDNUM_LANGUAGEPACK-osx.app $WD/output/edb-languagepack-$PG_VERSION_LANGUAGEPACK-$PG_BUILDNUM_LANGUAGEPACK-${BUILD_FAILED}osx.app || _die "Failed to rename the installer"
    ...
    ssh $PG_SSH_OSX "cd $PG_PATH_OSX/output; hdid edb-languagepack-$PG_VERSION_LANGUAGEPACK-$PG_BUILDNUM_LANGUAGEPACK-${BUILD_FAILED}osx.dmg" || _die "Failed to open the disk image (edb-languagepack-$PG_VERSION_LANGUAGEPACK-$PG_BUILDNUM_LANGUAGEPACK-${BUILD_FAILED}osx.dmg in remote host)"
