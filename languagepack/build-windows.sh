...
_prep_languagepack_windows() {
    ...
    then
       ...
       PG_LANGUAGEPACK_INSTALL_DIR_WIN="${PG_LANGUAGEPACK_INSTALL_DIR_WINDOWS}.build\\\\i386"
    else
       ARCH="windows-x64"
       PG_SSH_WIN=$PG_SSH_WINDOWS_X64
       PG_PATH_WIN=$PG_PATH_WINDOWS_X64
       PG_LANGUAGEPACK_INSTALL_DIR_WIN="${PG_LANGUAGEPACK_INSTALL_DIR_WINDOWS}.build\\\\x64"
    fi
...
_build_languagepack_windows() {
    ...
    then
       ...
       PG_LANGUAGEPACK_INSTALL_DIR_WIN="${PG_LANGUAGEPACK_INSTALL_DIR_WINDOWS}.build\\\\i386"
       PG_LANGUAGEPACK_STAGING_DIR="${PG_LANGUAGEPACK_INSTALL_DIR_WINDOWS}\\\\i386"
    else
       ARCH="windows-x64"
       PG_SSH_WIN=$PG_SSH_WINDOWS_X64
       PG_PATH_WIN=$PG_PATH_WINDOWS_X64
       ...
       PG_LANGUAGEPACK_INSTALL_DIR_WIN="${PG_LANGUAGEPACK_INSTALL_DIR_WINDOWS}.build\\\\x64"
       PG_LANGUAGEPACK_STAGING_DIR="${PG_LANGUAGEPACK_INSTALL_DIR_WINDOWS}\\\\x64"
    fi
...
EOT
  ...
  # Tcl/Tk Build
    ...
    echo "Removing last successful staging directory ($PG_LANGUAGEPACK_STAGING_DIR)"
    ssh $PG_SSH_WIN "cmd /c if EXIST $PG_LANGUAGEPACK_STAGING_DIR rd /S /Q $PG_LANGUAGEPACK_STAGING_DIR" || _die "Couldn't remove the last successful staging directory directory"
    ssh $PG_SSH_WIN "cmd /c mkdir $PG_LANGUAGEPACK_STAGING_DIR" || _die "Couldn't create the last successful staging directory"

    echo "Copying the complete build to the successful staging directory"
    ssh $PG_SSH_WIN "cmd /c xcopy /E /Q /Y ${PG_LANGUAGEPACK_INSTALL_DIR_WIN}\\\\* $PG_LANGUAGEPACK_STAGING_DIR\\\\" || _die "Couldn't copy the existing staging directory"

    ssh $PG_SSH_WIN "cmd /c echo PG_VERSION_LANGUAGEPACK=$PG_VERSION_LANGUAGEPACK >  $PG_LANGUAGEPACK_STAGING_DIR/versions-${ARCH}.sh" || _die "Failed to write languagepack version number into versions-windows.sh"
    ssh $PG_SSH_WIN "cmd /c echo PG_BUILDNUM_LANGUAGEPACK=$PG_BUILDNUM_LANGUAGEPACK >> $PG_LANGUAGEPACK_STAGING_DIR/versions-${ARCH}.sh" || _die "Failed to write languagepack build number into versions-windows.sh"
...
_postprocess_languagepack_windows() {
    ...
    rsync -av $PG_SSH_WIN:$PG_LANGUAGEPACK_INSTALL_DIR_WIN/versions-${ARCH}.sh  $WD/languagepack/staging/$ARCH || _die "Failed to copy versions-${ARCH}.sh"
