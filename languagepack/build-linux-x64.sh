...
_prep_languagepack_linux_x64() {
    ...
    ssh $PG_SSH_LINUX_X64 "rm -rf ${PG_LANGUAGEPACK_INSTALL_DIR_LINUX}.build" || _die "Couldn't remove the existing install directory"
    ...
    ssh $PG_SSH_LINUX_X64 "mkdir -p ${PG_LANGUAGEPACK_INSTALL_DIR_LINUX}.build" || _die "Couldn't create the install directory"
    chmod ugo+w $WD/languagepack/staging/linux-x64 || _die "Couldn't set the permissions on the staging directory"
    ssh $PG_SSH_LINUX_X64 "chmod ugo+w ${PG_LANGUAGEPACK_INSTALL_DIR_LINUX}.build" || _die "Couldn't set the permissions on the install directory"
}
...
_build_languagepack_linux_x64() {
    ...
    ssh $PG_SSH_LINUX_X64 "rm -rf $PG_LANGUAGEPACK_INSTALL_DIR_LINUX" || _die "Couldn't remove the last successful staging directory directory"
    ssh $PG_SSH_LINUX_X64 "mkdir -p $PG_LANGUAGEPACK_INSTALL_DIR_LINUX" || _die "Couldn't create the last successful staging directory"
    ssh $PG_SSH_LINUX_X64 "chmod ugo+w $PG_LANGUAGEPACK_INSTALL_DIR_LINUX" || _die "Couldn't set the permission on the last successful staging directory"

    echo "Copying the complete build to the successful staging directory"
    ssh $PG_SSH_LINUX_X64 "cp -rp ${PG_LANGUAGEPACK_INSTALL_DIR_LINUX}.build/* $PG_LANGUAGEPACK_INSTALL_DIR_LINUX" || _die "Couldn't copy the existing staging directory"

    ssh $PG_SSH_LINUX_X64 "echo PG_VERSION_LANGUAGEPACK=$PG_VERSION_LANGUAGEPACK > $PG_LANGUAGEPACK_INSTALL_DIR_LINUX/versions-linux-x64.sh" || _die "Failed to write languagepack version number into versions-linux-x64.sh"
    ssh $PG_SSH_LINUX_X64 "echo PG_BUILDNUM_LANGUAGEPACK=$PG_BUILDNUM_LANGUAGEPACK >> $PG_LANGUAGEPACK_INSTALL_DIR_LINUX/versions-linux-x64.sh" || _die "Failed to write languagepack build number into versions-linux-x64.sh"
...
_postprocess_languagepack_linux_x64() {
    ...
    ssh $PG_SSH_LINUX_X64 "cp -rp ${PG_LANGUAGEPACK_INSTALL_DIR_LINUX}.build/* $PG_PATH_LINUX_X64/languagepack/staging/linux-x64" || _die "Failed to copy the languagepack Source into the staging directory"
