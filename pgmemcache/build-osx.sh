...
_build_pgmemcache_osx() {
    ...
    CFLAGS="$PG_ARCH_OSX_FLAGS -arch ppc" ./configure prefix=$PG_PGHOME_OSX  --disable-static --disable-dependency-tracking || _die "Failed to configure libmemcached"
    mv libmemcached/libmemcached_config.h libmemcached/libmemcached_config_ppc.h || _die "Failed to rename config.h"

    CFLAGS="$PG_ARCH_OSX_FLAGS -arch i386" ./configure prefix=$PG_PGHOME_OSX  --disable-static --disable-dependency-tracking || _die "Failed to configure libmemcached"
    mv libmemcached/libmemcached_config.h libmemcached/libmemcached_config_i386.h || _die "Failed to rename config.h"

    CFLAGS="$PG_ARCH_OSX_FLAGS -arch i386 -arch ppc"  ./configure prefix=$PG_PGHOME_OSX  --disable-static --disable-dependency-tracking || _die "Failed to configure libmemcached"
    ...
    CFLAGS="$PG_ARCH_OSX_FLAGS -arch i386 -arch ppc" make  || _die "Failed to build libmemcached"
    ...
    PATH=$PATH:$PG_PGHOME_OSX/bin make  || _die "Failed to build pgmemcache"
