...
_prep_MigrationWizard_linux_x64() {
    ...
    mkdir -p $WD/MigrationWizard/staging/linux-x64/UserValidation || _die "Couldn't create the staging/UserValidation directory"
    mkdir -p $WD/MigrationWizard/staging/linux-x64/UserValidation/lib || _die "Couldn't create the staging/UserValidation/lib directory"
    chmod ugo+w $WD/MigrationWizard/staging/linux-x64 || _die "Couldn't set the permissions on the staging directory"
    chmod ugo+w $WD/MigrationWizard/staging/linux-x64/UserValidation || _die "Couldn't set the permissions on the staging/UserValidation directory"
    chmod ugo+w $WD/MigrationWizard/staging/linux-x64/UserValidation/lib || _die "Couldn't set the permissions on the staging/UserValidation/lib directory"

    echo "Copying validateUserClient scripts from MetaInstaller"
    cp $WD/MetaInstaller/scripts/linux/sysinfo.sh $WD/MigrationWizard/staging/linux-x64/UserValidation || _die "Couldn't copy MetaInstaller/scripts/linux/sysinfo.sh scripts"        
...
_build_MigrationWizard_linux_x64() {
    ...
    ssh $PG_SSH_LINUX_X64 "cd $PG_PATH_LINUX_X64/MigrationWizard/source/migrationwizard.linux-x64; JAVA_HOME=$PG_JAVA_HOME_LINUX_X64 $PG_ANT_HOME_LINUX_X64/bin/ant clean" || _die "Couldn't build the migrationwizard"
    ...
    ssh $PG_SSH_LINUX_X64 "cp -R /lib64/libssl.so* $PG_STAGING/UserValidation/lib" || _die "Failed to copy the dependency library (libssl)"
    ssh $PG_SSH_LINUX_X64 "cp -R /lib64/libcrypto.so* $PG_STAGING/UserValidation/lib" || _die "Failed to copy the dependency library (libcrypto)"

    # Build the validateUserClient binary
    if [ ! -f $WD/TuningWizard/source/tuningwizard.linux-x64/validateUser/validateUserClient.o ];
    then
        cp -R $WD/MetaInstaller/scripts/linux/validateUser $WD/MigrationWizard/source/migrationwizard.linux-x64/validateUser || _die "Failed to copy validateUser source files"
        ssh $PG_SSH_LINUX_X64 "cd $PG_PATH_LINUX_X64/MigrationWizard/source/migrationwizard.linux-x64/validateUser; gcc -DWITH_OPENSSL -I. -o validateUserClient.o WSValidateUserClient.c soapC.c soapClient.c stdsoap2.c -lssl -lcrypto" || _die "Failed to build the validateUserClient utility"
        cp $WD/MigrationWizard/source/migrationwizard.linux-x64/validateUser/validateUserClient.o $WD/MigrationWizard/staging/linux-x64/UserValidation/validateUserClient.o || _die "Failed to copy validateUserClient.o utility"
    else
       cp $WD/TuningWizard/source/tuningwizard.linux-x64/validateUser/validateUserClient.o $WD/MigrationWizard/staging/linux-x64/UserValidation/validateUserClient.o || _die "Failed to copy validateUserClient.o utility"
    fi

    chmod ugo+x $WD/MigrationWizard/staging/linux-x64/UserValidation/validateUserClient.o || _die "Failed to give execution permission to validateUserClient.o"
