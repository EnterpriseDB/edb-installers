...
_build_TuningWizard_linux_x64() {
    ...
    ssh $PG_SSH_LINUX_X64 "cp -R /usr/lib64/libgssapi_krb5.so* $PG_STAGING/TuningWizard/lib" || _die "Failed to copy the dependency library (libgssapi_krb5)"
    ssh $PG_SSH_LINUX_X64 "cp -R /usr/lib64/libkrb5.so* $PG_STAGING/TuningWizard/lib" || _die "Failed to copy the dependency library (libkrb5)"
...
_postprocess_TuningWizard_linux_x64() {
    ...
    cp resources/xdg/pg-launchTuningWizard.desktop staging/linux-x64/scripts/xdg/pg-launchTuningWizard.desktop || _die "Failed to copy a menu pick desktop"
