...
declare -a PACKAGES_ARR=(SERVER POSTGIS PGAGENT PGBOUNCER LANGUAGEPACK)
declare -a ENABLED_PKG_ARR=()
declare -a DECOUPLED_ARR=(PGBOUNCER LANGUAGEPACK)
...
usage()
{
        ...
        echo "    Examples:"
        echo "     $BASENAME -skipbuild 0  -packages "server,pgagent, pgbouncer""
...
./build.sh $SKIPBUILD 2>&1 | tee output/build-15.log
