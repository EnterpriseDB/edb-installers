...
_registration_preprocess_linux_x64()
{
  PG_REG_COMP_PLATFORM=linux-x64
  ...
  if [ x"$PG_REGISTRATION_COMP_BUILT_LINUX_X64" = x"" ]; then
...
_registration_build_linux_x64()
{
  PG_REG_COMP_PLATFORM=linux-x64
  PG_REG_COMP_PATH=$WD/registration/source/$PG_REG_COMP_PLATFORM
  PG_REG_COMP_STAGING=$WD/registration/staging/$PG_REG_COMP_PLATFORM
  PG_REG_COMP_HOST_PATH=$PG_PATH_LINUX_X64/registration/source/$PG_REG_COMP_PLATFORM
  PG_REG_COMP_HOST_STAGING=$PG_PATH_LINUX_X64/registration/staging/$PG_REG_COMP_PLATFORM

  cd $PG_REG_COMP_PATH

  if [ x"$PG_REGISTRATION_COMP_BUILT_LINUX_X64" = x"" ]; then
    echo "Building validateUserClient utility for registration..."
    ssh $PG_SSH_LINUX_X64 "cd $PG_REG_COMP_HOST_PATH; gcc -DWITH_OPENSSL -I. -o validateUserClient.o WSValidateUserClient.c soapC.c soapClient.c stdsoap2.c -lssl -lcrypto" || _die "Failed to build the validateUserClient utility for registration component"
    ...
    ssh $PG_SSH_LINUX_X64 "cp -R /usr/local/openssl/lib/libssl.so* $PG_REG_COMP_HOST_STAGING/lib" || _die "Failed to copy the dependency library (libssl)"
    ssh $PG_SSH_LINUX_X64 "cp -R /usr/local/openssl/lib/libcrypto.so* $PG_REG_COMP_HOST_STAGING/lib" || _die "Failed to copy the dependency library (libcrypto)"
    ...
    PG_REGISTRATION_COMP_BUILT_LINUX_X64=Done
  fi
}

_registration_postprocess_linux_x64()
{
  if [ $# -ne 1 ]; then
    _die "Wrong number of parameters while calling _registration_postprocess_linux_x64 (STAGING DIRECTORY)"
  fi
