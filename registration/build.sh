...
{
  if [ $# -ne 7 ]; then
    _die "Wrong number of parameters calling _registration_postprocess(STAGING DIRECTORY, COMPONENT NAME, VERSION VARIABLE, INI, REGISTRY_PREFIX, REGISTRY_PREFIX WINDOWS, COMONENT TYPE)"
  fi
  ...
  _replace @@COMPTYPE@@  "$7" registration_already_registered_parameter.xml
