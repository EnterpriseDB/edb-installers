...
if [ $BRANDED -ne 0 ];
then
    cp "$INSTALLDIR/mediaWiki/scripts/xdg/pg-postgresql.directory" "$INSTALLDIR/mediaWiki/scripts/xdg/pg-$BRANDING_STR.directory"
fi
...
"$INSTALLDIR/mediaWiki/installer/xdg/xdg-desktop-menu" install --mode system  \
      "$INSTALLDIR/mediaWiki/scripts/xdg/pg-$BRANDING_STR.directory"  \
