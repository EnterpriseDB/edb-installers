...
function start
{
    PID=\`ps -axef | grep 'java -XX:-UsePerfData -Djava.awt.headless=true -jar edb-repserver.jar pubserver $PUBPORT' | grep -v grep | awk '{print \$2}'\`

    if [ "x\$PID" = "x" ];
    then
       su $SYSTEM_USER -c "cd '$INSTALL_DIR/bin'; nohup '$JAVA' -XX:-UsePerfData -Djava.awt.headless=true -jar edb-repserver.jar pubserver $PUBPORT > /dev/null 2>&1 &"
