...
# Hyperic server shortcut creation script for Linux
...
# In case of serverctl.sh and agentctl.sh: We do not want to change PGHYPERIC_VERSION_STR to X_Y_Z from X.Y.Z because path is server-4.2.0 not server-4_2_0. But we want to replace PGHYPERIC_INSTALLDIR. So revert back one  change done by fix_up function.
...
_fixup_file "$INSTALLDIR/scripts/xdg/pghyperic-$VERSION_STR.directory"
_fixup_file "$INSTALLDIR/scripts/xdg/pghyperic-launch-$VERSION_STR.desktop"
_fixup_file "$INSTALLDIR/scripts/xdg/pghyperic-start-$VERSION_STR.desktop"
_fixup_file "$INSTALLDIR/scripts/xdg/pghyperic-stop-$VERSION_STR.desktop"
_fixup_file "$INSTALLDIR/scripts/xdg/pghyperic-agent-start-$VERSION_STR.desktop"
_fixup_file "$INSTALLDIR/scripts/xdg/pghyperic-agent-stop-$VERSION_STR.desktop"
...
"$INSTALLDIR/installer/xdg/xdg-desktop-menu" install --mode system --noupdate \
	  ...
	  "$INSTALLDIR/scripts/xdg/pghyperic-agent-stop-$VERSION_STR.desktop" || _warn "Failed to create the top level menu Hyperic"
