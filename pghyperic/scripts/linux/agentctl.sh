...
if [ $USE_SUDO != "1" ];
then
    su - -c "PGHYPERIC_INSTALLDIR/agent-PGHYPERIC_VERSION_STR/bin/hq-agent.sh $action"
else
    sudo  PGHYPERIC_INSTALLDIR/agent-PGHYPERIC_VERSION_STR/bin/hq-agent.sh $action
fi
