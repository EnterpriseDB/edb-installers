...
cat <<EOT-SLONY > $WD/Slony/build-slony.sh
    ...
    do
      ...
      CFLAGS="$PG_ARCH_OSX_CFLAGS -arch \${ARCH}" PATH="$PG_PGHOME_OSX/bin:$PATH" DYLD_LIBRARY_PATH="$DYLD_LIBRARY_PATH:$PG_PGHOME_OSX/lib" ./configure --prefix=$PG_PGHOME_OSX --with-pgconfigdir=$PG_PGHOME_OSX/bin --with-pgport=yes || _die "Failed to configure slony for intel \${ARCH}"
    ...
    CFLAGS="$PG_ARCH_OSX_CFLAGS \${ARCH_FLAGS}" PATH="$PG_PGHOME_OSX/bin:$PATH" DYLD_LIBRARY_PATH="$DYLD_LIBRARY_PATH:$PG_PGHOME_OSX/lib" ./configure --prefix=$PG_PGHOME_OSX --with-pgconfigdir=$PG_PGHOME_OSX/bin --with-pglibdir=$PG_PGHOME_OSX/lib --with-pgport=yes || _die "Failed to configure slony for Universal"
...
_postprocess_Slony_osx() {
    ...
    ssh $PG_SSH_OSX_NOTARY "cd $PG_PATH_OSX_NOTARY; ./notarize_apps.sh slony-pg$PG_CURRENT_VERSION-$PG_VERSION_SLONY-$PG_BUILDNUM_SLONY-${BUILD_FAILED}osx.zip slony" || _die "Failed to notarize the app"
