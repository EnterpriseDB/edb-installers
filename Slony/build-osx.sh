...
cat <<EOT-SLONY > $WD/Slony/build-slony.sh
    ...
    CFLAGS="$PG_ARCH_OSX_CFLAGS -arch x86_64 -g" PATH="$PG_PGHOME_OSX/bin:$PATH" DYLD_LIBRARY_PATH="$DYLD_LIBRARY_PATH:$PG_PGHOME_OSX/lib" ./configure --prefix=$PG_PGHOME_OSX --with-pgconfigdir=$PG_PGHOME_OSX/bin || _die "Failed to configure slony for intel x86_64"
