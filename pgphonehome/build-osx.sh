...
_build_pgphonehome_osx() {
    ...
    OLD_DLL=`otool -L $PG_PATH_OSX/pgphonehome/staging/osx/instscripts/psql | grep @loader_path/../lib |  grep -v ":" | awk '{ print $1 }' `
    NEW_DLL=`echo $OLD_DLL | sed -e "s^@loader_path/../lib/^^g"`

    install_name_tool -change "$OLD_DLL" "$NEW_DLL" "$PG_PATH_OSX/pgphonehome/staging/osx/instscripts/psql"
