...
./configure --enable-debug --with-pgconfig=$POSTGRES_REMOTE_PATH/bin/pg_config --with-geosconfig=/opt/local/Current/bin/geos-config --with-libiconv=/opt/local/Current --with-projdir=/opt/local/Current --with-jsondir=/opt/local/Current || _die "Failed to configure postgis"
...
_postprocess_PostGIS_linux_x64() {
    ...
    cp resources/pg-launchPostGISJDBCDocs.png staging/linux-x64/scripts/images/pg-launchPostGISJDBCDocs-$POSTGIS_VERSION_STR-$PG_VERSION_STR.png || _die "Failed to copy the menu pick image (resources/pg-launchJdbcDocs.icns)"
    ...
    cp resources/xdg/pg-launchPostGISJDBCDocs.desktop staging/linux-x64/scripts/xdg/pg-launchPostGISJDBCDocs-$POSTGIS_VERSION_STR-$PG_VERSION_STR.desktop || _die "Failed to copy a menu pick desktop"
    ...
    mv $WD/output/postgis-pg$PG_CURRENT_VERSION-$PG_VERSION_POSTGIS-$PG_BUILDNUM_POSTGIS-linux-x64.run $WD/output/postgis-pg$PG_CURRENT_VERSION-$PG_VERSION_POSTGIS-$PG_BUILDNUM_POSTGIS-${BUILD_FAILED}linux-x64.run
