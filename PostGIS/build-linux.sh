...
_build_PostGIS_linux() {
    ...
    ssh $PG_SSH_LINUX "cd $PG_PATH_LINUX/PostGIS/source/postgis.linux; export LD_LIBRARY_PATH=$PG_PGHOME_LINUX/lib:$PG_CACHING/proj-$PG_TARBALL_PROJ.linux/lib:$PG_CACHING/geos-$PG_TARBALL_GEOS.linux/lib:\$LD_LIBRARY_PATH; make; make comments" || _die "Failed to build postgis"
    ssh $PG_SSH_LINUX "cd $PG_PATH_LINUX/PostGIS/source/postgis.linux; export LD_LIBRARY_PATH=$PG_PGHOME_LINUX/lib:$PG_CACHING/proj-$PG_TARBALL_PROJ.linux/lib:$PG_CACHING/geos-$PG_TARBALL_GEOS.linux/lib:\$LD_LIBRARY_PATH; make install; make comments-install" || _die "Failed to install postgis"
