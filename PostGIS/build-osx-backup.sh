...
cat <<EOT-POSTGIS >> $WD/PostGIS/build-postgis.sh
    _build_geos() {
    cd $PG_PATH_OSX/PostGIS/source/geos.osx || _die "Failed to change to the geos source directory (PostGIS/source/geos.osx)"
    ...
    CFLAGS="$PG_ARCH_OSX_CFLAGS -arch x86_64" CXXFLAGS="$PG_ARCH_OSX_CXXFLAGS -arch x86_64" MACOSX_DEPLOYMENT_TARGET=10.5 ./configure --prefix=$PG_CACHING/geos-$PG_TARBALL_GEOS.osx --disable-dependency-tracking || _die "Failed to configure Geos for x86_64"
    mv source/headers/config.h source/headers/config_x86_64.h

    echo "Configuring the Geos source tree for Universal"
    CFLAGS="$PG_ARCH_OSX_CFLAGS  -arch i386 -arch x86_64" CXXFLAGS="$PG_ARCH_OSX_CXXFLAGS -arch i386  -arch x86_64" MACOSX_DEPLOYMENT_TARGET=10.5 ./configure --prefix=$PG_CACHING/geos-$PG_TARBALL_GEOS.osx --disable-dependency-tracking || _die "Failed to configure Geos for Universal"
    ...
    echo "  #error "Dont support ppc architecture"" >> source/headers/config.h
    ...
    echo "Building Geos"
    MACOSX_DEPLOYMENT_TARGET=10.5 make LDFLAGS="-arch i386  -arch x86_64" -j 2 || _die "Failed to build Geos"
...
cat <<EOT-POSTGIS >> $WD/PostGIS/build-postgis.sh
    ...
    CFLAGS="$PG_ARCH_OSX_CFLAGS -arch x86_64" MACOSX_DEPLOYMENT_TARGET=10.5 ./configure --prefix=$PG_CACHING/proj-$PG_TARBALL_PROJ.osx --disable-dependency-tracking || _die "Failed to configure Proj for x86_64"
    mv src/proj_config.h src/proj_config_x86_64.h

    echo "Configuring the Proj source tree for Universal"
    CFLAGS="$PG_ARCH_OSX_CFLAGS  -arch i386 -arch x86_64" MACOSX_DEPLOYMENT_TARGET=10.5 ./configure --prefix=$PG_CACHING/proj-$PG_TARBALL_PROJ.osx --disable-dependency-tracking || _die "Failed to configure Proj for Universal"

    # Create a replacement config.h that will pull in the appropriate architecture-specific one:
    echo "#ifdef __BIG_ENDIAN__" > src/proj_config.h
    echo "  #error "Dont support ppc architecture"" >> src/proj_config.h
...
cat <<EOT-POSTGIS >> $WD/PostGIS/build-postgis.sh
    ...
    make install DESTDIR=$PG_PATH_OSX/PostGIS/staging/osx/PostGIS bindir=/bin pkglibdir=/lib datadir=/share PGSQL_DOCDIR=$PG_PATH_OSX/PostGIS/staging/osx/PostGIS/doc PGSQL_MANDIR=$PG_PATH_OSX/PostGIS/staging/osx/PostGIS/man PGSQL_SHAREDIR=$PG_PATH_OSX/PostGIS/staging/osx/PostGIS/share/postgresql || _die "Failed to install PostGIS"
    make comments-install DESTDIR=$PG_PATH_OSX/PostGIS/staging/osx/PostGIS bindir=/bin pkglibdir=/lib datadir=/share PGSQL_DOCDIR=$PG_PATH_OSX/PostGIS/staging/osx/PostGIS/doc PGSQL_MANDIR=$PG_PATH_OSX/PostGIS/staging/osx/PostGIS/man PGSQL_SHAREDIR=$PG_PATH_OSX/PostGIS/staging/osx/PostGIS/share/postgresql || _die "Failed to install PostGIS"
    ...
    make install DESTDIR=$PG_PATH_OSX/PostGIS/staging/osx/PostGIS bindir=/bin pkglibdir=/lib datadir=/share PGSQL_DOCDIR=/doc PGSQL_MANDIR=/man PGSQL_SHAREDIR=$PG_PATH_OSX/PostGIS/staging/osx/PostGIS/share/postgresql || _die "Failed to install PostGIS-doc"
...
cat <<EOT-POSTGIS >> $WD/PostGIS/build-postgis.sh
    if [ ! -e $PG_PATH_OSX/PostGIS/caching/osx/proj-$PG_TARBALL_PROJ.osx ];
    then
      # Building proj
      _build_proj
    ...
    if [ ! -e $PG_PATH_OSX/PostGIS/caching/osx/geos-$PG_TARBALL_GEOS.osx ];
    then
      # Building geos
      _build_geos
    fi
...
_change_so_refs() {
    ...
    _change_so_refs $PG_PATH_OSX/PostGIS/staging/osx/geos lib @loader_path/..
    _change_so_refs $PG_PATH_OSX/PostGIS/staging/osx/proj lib @loader_path/..
...
_postprocess_PostGIS_osx() {
    ...
    cp -R $WD/PostGIS/scripts/osx/pg-launchJdbcDocs.applescript.in $PG_STAGING/scripts/pg-launchJdbcDocs.applescript || _die "Failed to copy the launch script (scripts/osx/pg-launchJdbcDocs.applescript.in)"
    cp -R $WD/PostGIS/scripts/osx/pg-launchPostGISDocs.applescript.in $PG_STAGING/scripts/pg-launchPostGISDocs.applescript || _die "Failed to copy the launch script (scripts/osx/pg-launchPostGISDocs.applescript.in)"

    # Copy in the menu pick images 
    mkdir -p $WD/PostGIS/staging/osx/scripts/images || _die "Failed to create a directory for the menu pick images"
    cp $WD/PostGIS/resources/pg-launchPostGISDocs.icns $WD/PostGIS/staging/osx/scripts/images || _die "Failed to copy the menu pick image (resources/pg-launchPostGISDocs.icns)"
    cp $WD/PostGIS/resources/pg-launchPostGISJDBCDocs.icns $WD/PostGIS/staging/osx/scripts/images || _die "Failed to copy the menu pick image (resources/pg-launchJdbcDocs.icns)"
