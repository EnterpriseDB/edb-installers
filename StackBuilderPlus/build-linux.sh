...
_build_stackbuilderplus_linux() {
    ...
    cp $WD/StackBuilderPlus/source/updatemanager.linux/UpdateManager $WD/StackBuilderPlus/staging/linux/bin

    echo "Copying dependent libraries to staging directory (linux)"
    ssh $PG_SSH_LINUX "cp /usr/lib/libQtXml.so.* $PG_PATH_LINUX/StackBuilderPlus/staging/linux/lib" || _die "Failed to copy dependent library (libQtXml.so) in staging directory (linux)"
    ssh $PG_SSH_LINUX "cp /usr/lib/libQtNetwork.so.* $PG_PATH_LINUX/StackBuilderPlus/staging/linux/lib" || _die "Failed to copy dependent library (libQtNetwork.so) in staging directory (linux)"
    ssh $PG_SSH_LINUX "cp /usr/lib/libQtCore.so.* $PG_PATH_LINUX/StackBuilderPlus/staging/linux/lib" || _die "Failed to copy dependent library (libQtCore.so) in staging directory (linux)"
    ssh $PG_SSH_LINUX "cp /usr/lib/libQtGui.so.* $PG_PATH_LINUX/StackBuilderPlus/staging/linux/lib" || _die "Failed to copy dependent library (libQtGui.so) in staging directory (linux)"
    ...
    ssh $PG_SSH_LINUX "cp /usr/lib/libpangoft2-1.0.so* $PG_PATH_LINUX/StackBuilderPlus/staging/linux/lib" || _die "Failed to copy dependent library (libpangoft2-1.0.so) in staging directory (linux)"
    ssh $PG_SSH_LINUX "cp /usr/lib/libpangoxft-1.0.so* $PG_PATH_LINUX/StackBuilderPlus/staging/linux/lib" || _die "Failed to copy dependent library (libpangoxft-1.0.so) in staging directory (linux)"
    ssh $PG_SSH_LINUX "cp /usr/lib/libpangox-1.0.so* $PG_PATH_LINUX/StackBuilderPlus/staging/linux/lib" || _die "Failed to copy dependent library (libpangox-1.0.so) in staging directory (linux)"
    ssh $PG_SSH_LINUX "cp /usr/lib/libpango-1.0.so* $PG_PATH_LINUX/StackBuilderPlus/staging/linux/lib" || _die "Failed to copy dependent library (libpango-1.0.so) in staging directory (linux)"
...
_postprocess_stackbuilderplus_linux() { 
    ...
    cp scripts/linux/launchSBPUpdateMonitor.sh staging/linux/scripts/launchSBPUpdateMonitor.sh || _die "Failed to copy the launch scripts (scripts/linux/launchSBPUpdateMonitor.sh)"
    ...
    cp resources/xdg/edb-sbp-update-monitor.desktop staging/linux/scripts/xdg/edb-sbp-update-monitor.desktop || _die "Failed to copy the startup pick desktop"
