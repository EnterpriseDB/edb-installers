...
_build_stackbuilderplus_linux_x64() {
    ...
    ssh $PG_SSH_LINUX_X64 "cp /usr/local/openssl/lib/libssl.so* $PG_PATH_LINUX_X64/StackBuilderPlus/staging/linux-x64/lib" || _die "Failed to copy dependent library (libssl.so) in staging directory (linux-x64)"
    ssh $PG_SSH_LINUX_X64 "cp /usr/local/openssl/lib/libcrypto.so* $PG_PATH_LINUX_X64/StackBuilderPlus/staging/linux-x64/lib" || _die "Failed to copy dependent library (libcrypto.so) in staging directory (linux-x64)"
