...
_build_stackbuilderplus_linux_x64() {
    ...
    ssh $PG_SSH_LINUX_X64 "cp /usr/local/lib/libuuid.so* $PG_PATH_LINUX_X64/StackBuilderPlus/staging/linux-x64/lib" || _die "Failed to copy dependent library (libpangoft2-1.0.so) in staging directory (linux-x64)"
