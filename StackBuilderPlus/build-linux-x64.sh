...
_build_stackbuilderplus_linux_x64() {
    ...
    ssh $PG_SSH_LINUX_X64 "cd $PG_PATH_LINUX_X64/StackBuilderPlus/source/StackBuilderPlus.linux-x64/; cmake -D CMAKE_BUILD_TYPE:STRING=Release -D WX_CONFIG_PATH:FILEPATH=/opt/local/Current/bin/wx-config -D WX_DEBUG:BOOL=OFF -D WX_STATIC:BOOL=ON -D CMAKE_INSTALL_PREFIX:PATH=$PG_PATH_LINUX_X64/StackBuilderPlus/staging/linux-x64 ."
    ...
    ssh $PG_SSH_LINUX_X64 "cp /opt/local/Current/lib64/libssl.so* $PG_PATH_LINUX_X64/StackBuilderPlus/staging/linux-x64/lib" || _die "Failed to copy dependent library (libssl.so) in staging directory (linux-x64)"
    ssh $PG_SSH_LINUX_X64 "cp /opt/local/Current/lib64/libcrypto.so* $PG_PATH_LINUX_X64/StackBuilderPlus/staging/linux-x64/lib" || _die "Failed to copy dependent library (libcrypto.so) in staging directory (linux-x64)"
    ssh $PG_SSH_LINUX_X64 "cp /opt/local/Current/lib64/libexpat.so* $PG_PATH_LINUX_X64/StackBuilderPlus/staging/linux-x64/lib" || _die "Failed to copy dependent library (libexpat.so) in staging directory (linux-x64)"
    ssh $PG_SSH_LINUX_X64 "cp /opt/local/Current/lib64/libgssapi_krb5.so* $PG_PATH_LINUX_X64/StackBuilderPlus/staging/linux-x64/lib" || _die "Failed to copy dependent library (libgssapi_krb5.so) in staging directory (linux-x64)"
    ssh $PG_SSH_LINUX_X64 "cp /opt/local/Current/lib64/libkrb5.so* $PG_PATH_LINUX_X64/StackBuilderPlus/staging/linux-x64/lib" || _die "Failed to copy dependent library (libkrb5.so) in staging directory (linux-x64)"
    ssh $PG_SSH_LINUX_X64 "cp /opt/local/Current/lib64/libcom_err.so* $PG_PATH_LINUX_X64/StackBuilderPlus/staging/linux-x64/lib" || _die "Failed to copy dependent library (libcom_err.so) in staging directory (linux-x64)"
    ssh $PG_SSH_LINUX_X64 "cp /opt/local/Current/lib64/libk5crypto.so* $PG_PATH_LINUX_X64/StackBuilderPlus/staging/linux-x64/lib" || _die "Failed to copy dependent library (libk5crypto.so) in staging directory (linux-x64)"
    ssh $PG_SSH_LINUX_X64 "cp /opt/local/Current/lib64/libjpeg.so* $PG_PATH_LINUX_X64/StackBuilderPlus/staging/linux-x64/lib" || _die "Failed to copy dependent library (libjpeg.so) in staging directory (linux-x64)"
    ssh $PG_SSH_LINUX_X64 "cp /opt/local/Current/lib64/libtiff.so* $PG_PATH_LINUX_X64/StackBuilderPlus/staging/linux-x64/lib" || _die "Failed to copy dependent library (libtiff.so) in staging directory (linux-x64)"
    ssh $PG_SSH_LINUX_X64 "cp /opt/local/Current/lib64/libz.so* $PG_PATH_LINUX_X64/StackBuilderPlus/staging/linux-x64/lib" || _die "Failed to copy dependent library (libz.so) in staging directory (linux-x64)"
    ssh $PG_SSH_LINUX_X64 "cp /opt/local/Current/lib64/libfreetype.so* $PG_PATH_LINUX_X64/StackBuilderPlus/staging/linux-x64/lib" || _die "Failed to copy dependent library (libfreetype.so) in staging directory (linux-x64)"
    ssh $PG_SSH_LINUX_X64 "cp /opt/local/Current/lib64/libfontconfig.so* $PG_PATH_LINUX_X64/StackBuilderPlus/staging/linux-x64/lib" || _die "Failed to copy dependent library (libfontconfig.so) in staging directory (linux-x64)"
