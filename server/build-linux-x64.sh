...
_build_server_linux_x64() {
    ...
    ssh $PG_SSH_LINUX_X64 "cd $PG_PATH_LINUX_X64/server/source/postgres.linux-x64; export LD_LIBRARY_PATH=/usr/local/openssl/lib:/usr/local/lib:$LD_LIBRARY_PATH; make" || _die "Failed to build postgres" 
    ssh $PG_SSH_LINUX_X64 "cd $PG_PATH_LINUX_X64/server/source/postgres.linux-x64; export LD_LIBRARY_PATH=/usr/local/openssl/lib:/usr/local/lib:$LD_LIBRARY_PATH; make install" || _die "Failed to install postgres"
    ...
    _process_dependent_libs "$PG_STAGING/bin" "$PG_STAGING/lib" "libz.so"  
    _process_dependent_libs "$PG_STAGING/bin" "$PG_STAGING/lib" "libiconv.so"  
...
age"
    ...
    cp resources/xdg/pg-restart.desktop staging/linux-x64/scripts/xdg/pg-restart-$PG_VERSION_STR.desktop || _die "Failed to copy a menu pick"
    ...
    cp resources/xdg/pg-start.desktop staging/linux-x64/scripts/xdg/pg-start-$PG_VERSION_STR.desktop || _die "Failed to copy a menu pick"
    cp resources/xdg/pg-stop.desktop staging/linux-x64/scripts/xdg/pg-stop-$PG_VERSION_STR.desktop || _die "Failed to copy a menu pick"
