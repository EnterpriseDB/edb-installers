...
EOT
	...
	ssh $PG_SSH_WINDOWS "cmd /c copy $PG_PATH_WINDOWS\\\\postgres.windows\\\\contrib\\\\pldebugger\\\\targetinfo.dll $PG_PATH_WINDOWS\\\\output\\\\lib" || _die "Failed to copy the debugger target info library on the windows build host"
	...
	cp $WD/server/source/postgresql-$PG_TARBALL_POSTGRESQL/contrib/pldebugger/uninstall_pldbgapi.sql $WD/server/staging/windows/share/contrib
