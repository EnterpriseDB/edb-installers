...
start()
{
	...
	su -s /bin/sh - $USERNAME -c "LD_LIBRARY_PATH=$INSTALLDIR/lib $INSTALLDIR/bin/pg_ctl -w start -D \"$DATADIR\" -l \"$DATADIR/pg_log/startup.log\""
...
stop()
{
	...
	su -s /bin/sh - $USERNAME -c "LD_LIBRARY_PATH=$INSTALLDIR/lib $INSTALLDIR/bin/pg_ctl stop -m fast -w -D \"$DATADIR\""
	if [ \$? -eq 0 ];
        then
                rm -f \$LOCKFILE
        fi
...
restart()
{
	...
	su -s /bin/sh - $USERNAME -c "LD_LIBRARY_PATH=$INSTALLDIR/lib $INSTALLDIR/bin/pg_ctl -w restart -D \"$DATADIR\" -l \"$DATADIR/pg_log/startup.log\" -m fast"
...
reload()
{
	...
	su -s /bin/sh - $USERNAME -c "LD_LIBRARY_PATH=$INSTALLDIR/lib $INSTALLDIR/bin/pg_ctl reload -D \"$DATADIR\""
}
...
case "\$1" in
  ...
  condrestart)
        ...
        ;;
  status)
        su -s /bin/sh - $USERNAME -c "LD_LIBRARY_PATH=$INSTALLDIR/lib $INSTALLDIR/bin/pg_ctl status -D \"$DATADIR\""
        ;;
  *)
