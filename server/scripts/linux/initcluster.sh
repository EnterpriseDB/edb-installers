...
if [ $LOCALE = "DEFAULT" ];
then
    su - $OSUSERNAME -c "LD_LIBRARY_PATH=$INSTALLDIR/lib:$LD_LIBRARY_PATH $INSTALLDIR/bin/initdb --pwfile $INSTALLDIR/installer/server/initdbpw.$$ -A md5 -U \"$SUPERNAME\" -D \"$DATADIR\"" || _die "Failed to initialise the database cluster with initdb"
else
    su - $OSUSERNAME -c "LD_LIBRARY_PATH=$INSTALLDIR/lib:$LD_LIBRARY_PATH $INSTALLDIR/bin/initdb --pwfile $INSTALLDIR/installer/server/initdbpw.$$ --locale=$LOCALE -A md5 -U \"$SUPERNAME\" -D \"$DATADIR\"" || _die "Failed to initialise the database cluster with initdb"
fi
