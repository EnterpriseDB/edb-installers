...
chown $SUPERNAME:$SUPERNAME $INSTALLDIR/installer/server/initdbpw.$$ || _die "Failed to set the ownership of the initdb password file ($INSTALLDIR/installer/server/initdbpw.$$)"
...
if [ $LOCALE = "DEFAULT" ];
then
    su - $SUPERNAME -c "LD_LIBRARY_PATH=$INSTALLDIR/lib $INSTALLDIR/bin/initdb --pwfile $INSTALLDIR/installer/server/initdbpw.$$ -A md5 -D \"$DATADIR\"" || _die "Failed to initialise the database cluster with initdb"
else
    su - $SUPERNAME -c "LD_LIBRARY_PATH=$INSTALLDIR/lib $INSTALLDIR/bin/initdb --pwfile $INSTALLDIR/installer/server/initdbpw.$$ --locale=$LOCALE -A md5 -D \"$DATADIR\"" || _die "Failed to initialise the database cluster with initdb"
fi
