...
chown $OSUSERNAME:daemon /tmp/initdbpw.$$ || _die "Failed to set the ownership of the initdb password file (/tmp/initdbpw.$$)"
...
if [ $LOCALE = "DEFAULT" ];
then
    su - $OSUSERNAME -c "$INSTALLDIR/bin/initdb --pwfile /tmp/initdbpw.$$ --encoding=utf8 -A md5 -U \"$SUPERNAME\" -D \"$DATADIR\"" || _die "Failed to initialise the database cluster with initdb"
else
...
chown $OSUSERNAME:daemon "$DATADIR/pg_log" || _die "Failed to set the ownership of the log directory ($DATADIR/pg_log)"
