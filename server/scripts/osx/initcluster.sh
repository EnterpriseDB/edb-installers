...
if [ $LOCALE = "DEFAULT" ];
then
    sudo -u $OSUSERNAME "$INSTALLDIR"/bin/initdb --pwfile "$INSTALLDIR"/installer/server/initdbpw.$$ --encoding=utf8 -A md5 -U "$SUPERNAME" -D "$DATADIR" || _die "Failed to initialise the database cluster with initdb"
else
    sudo -u $OSUSERNAME "$INSTALLDIR"/bin/initdb --pwfile "$INSTALLDIR"/installer/server/initdbpw.$$ --locale=$LOCALE -A md5 -U "$SUPERNAME" -D "$DATADIR" || _die "Failed to initialise the database cluster with initdb"
fi	
