...
echo "Installing pl/pgsql in the postgres and template databases..."
su - $USERNAME -c "PGPASSFILE=/tmp/pgpass.$$ $INSTALLDIR/bin/psql -p $PORT -c 'CREATE LANGUAGE plpgsql;' postgres" || _warn "Failed to install pl/pgsql in the 'postgres' database"
