...
EOT
    ...
    PATH=/usr/local/bin:$PATH CFLAGS="$PG_ARCH_OSX_CFLAGS -arch i386 -arch x86_64 -O2" make -j4 || _die "Failed to build postgres"
    ...
    cp /usr/local/include/iconv.h $WD/server/staging/osx/include || _die "Failed to copy the required header"
...
#    cp scripts/osx/doc-pljava-readme.applescript.in staging/osx/scripts/doc-pljava-readme.applescript || _die "Failed to to the menu pick script (scripts/osx/doc-pljava-readme.applescript.in)"
    ...
    tar -jxvf server.img.tar.bz2 || _die "Failed to extract server.img"
