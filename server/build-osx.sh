...
_build_server_osx() {
    ...
    cd $WD/server/source/postgres.osx/src/pl/plpython

    _replace "-framework" "-F/System/Library/Frameworks -framework" Makefile || _die "Failed to replace the flag to force to use the System's framework for plpython"
...
EOT
    ...
    cp /usr/local/include/ossp/uuid.h $WD/server/staging/osx/include || _die "Failed to copy the required header"
...
_postprocess_server_osx() {
    ...
    rm server.img.tar.bz2 || _die "Failed to remove server.img ."
   ...
   # Switch to regression directory  
    ...
    then
      ...
      rm -rf regress  || _die "Couldn't remove the existing regress source directory (/buildfarm/PG90/regress)"
    fi

    # Copy the regress folder into Regression Setup folder /buildfarm/PG90/src/test/
    cp -rf $WD/server/source/postgres.osx/src/test/regress /buildfarm/PG90/src/test/;

    # Delete the old installers present in /buildfarm/PG90/installers/
    rm -f /buildfarm/PG90/installers/*  
    
    # Copy the DBServer installer into Regression Setup folder /buildfarm/PG90/installers
    cp -p $WD/output/postgresql-$PG_PACKAGE_VERSION-osx.dmg /buildfarm/PG90/installers/ || _die "Unable to copy installers to Linux-64 /buildfarm/PG90/installers/ folder."
