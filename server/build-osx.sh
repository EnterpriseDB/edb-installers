...
_build_server_osx() {
    ...
    ssh $PG_SSH_OSX "cd $PG_PATH_OSX/server/source/postgres.osx/; PG_SYSROOT=$PG_SYSROOT PATH=/opt/local/Current/bin:$PATH CFLAGS='$PG_ARCH_OSX_CFLAGS -O2' LDFLAGS=\"-L/opt/local/Current/lib\" PYTHON=$PG_PYTHON_OSX/bin/python3 TCL_CONFIG_SH=$PG_TCL_OSX/lib/tclConfig.sh PERL=$PG_PERL_OSX/bin/perl XML2_CONFIG=/opt/local/Current/bin/xml2-config ICU_LIBS=\"-L/opt/local/Current/lib -licuuc -licudata -licui18n\" ICU_CFLAGS=\"-I/opt/local/Current/include\" LZ4_CFLAGS=\"-I/opt/local/Current/include\" LZ4_LIBS=\"-L/opt/local/Current/lib\" ./configure --with-icu --enable-debug  --prefix=$PG_STAGING --with-ldap --with-openssl --with-perl --with-python --with-tcl --with-bonjour --with-pam --with-libxml --with-uuid=e2fs --with-includes=/opt/local/Current/include/libxml2:/opt/local/Current/include:/opt/local/Current/include/security:/opt/local/Current/include/openssl/ --docdir=$PG_STAGING/doc/postgresql --with-libxslt --with-libedit-preferred --with-gssapi --with-lz4" || _die "Failed to configure postgres for universal support"
