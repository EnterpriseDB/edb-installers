...
_build_server_osx() {
    ...
    ssh $PG_SSH_OSX "cd $PG_PATH_OSX/server/source/postgres.osx/; PG_SYSROOT=$PG_SYSROOT PATH=/opt/local/Current/bin:$PATH CFLAGS='$PG_ARCH_OSX_CFLAGS -arch x86_64 -O2' LDFLAGS=\"-L/opt/local/Current/lib\" PYTHON=$PG_PYTHON_OSX/bin/python3 TCL_CONFIG_SH=$PG_TCL_OSX/lib/tclConfig.sh PERL=$PG_PERL_OSX/bin/perl XML2_CONFIG=/opt/local/Current/bin/xml2-config ICU_LIBS=\"-L/opt/local/Current/lib -licuuc -licudata -licui18n\" ICU_CFLAGS=\"-I/opt/local/Current/include\" ./configure --with-icu --enable-debug --host=x86_64-apple-darwin --prefix=$PG_STAGING --with-ldap --with-openssl --with-perl --with-python --with-tcl --with-bonjour --with-pam --with-libxml --with-uuid=e2fs --with-includes=/opt/local/Current/include/libxml2:/opt/local/Current/include:/opt/local/Current/include/security:/opt/local/Current/include/openssl/ --docdir=$PG_STAGING/doc/postgresql --with-libxslt --with-libedit-preferred --with-gssapi" || _die "Failed to configure postgres for x86_64"
    ...
    ssh $PG_SSH_OSX "cd $PG_PATH_OSX/server/source/stackbuilder.osx; PATH=/opt/local/Current/bin:$PATH $PG_CMAKE_OSX -D CMAKE_OSX_DEPLOYMENT_TARGET:STRING=$MACOSX_MIN_VERSION -D CURL_ROOT:PATH=/opt/local/Current -D CMAKE_BUILD_TYPE:STRING=Release -D WX_CONFIG_PATH:FILEPATH=/opt/local/Current/bin/wx-config -D WX_DEBUG:BOOL=OFF -D WX_STATIC:BOOL=OFF -D WX_VERSION=3.2 -D CMAKE_OSX_SYSROOT:FILEPATH=$PG_SYSROOT -D CMAKE_OSX_ARCHITECTURES:STRING=x86_64 ."  || _die "Failed to configure StackBuilder"
    ...
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current/lib/libkrb5*.dylib $PG_STAGING/lib/" || _die "Failed to copy the latest libkrb5"
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current/lib/libgss*.dylib $PG_STAGING/lib/" || _die "Failed to copy the latest libgssapi"
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current/lib/libk5*.dylib $PG_STAGING/lib/" || _die "Failed to copy the latest libk5"
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current/lib/libcom*.dylib $PG_STAGING/lib/" || _die "Failed to copy the latest libcom"
