...
_build_server_osx() {
    ...
    ssh $PG_SSH_OSX "cd $PG_PATH_OSX/server/source/postgres.osx/; PATH=/opt/local/Current_v18/bin:$PATH CFLAGS='$PG_ARCH_OSX_CFLAGS -O2' LDFLAGS=\"-L/opt/local/Current_v18/lib\" PYTHON=$PG_PYTHON_OSX/bin/python3 TCL_CONFIG_SH=$PG_TCL_OSX/lib/tclConfig.sh PERL=$PG_PERL_OSX/bin/perl XML2_CONFIG=/opt/local/Current_v18/bin/xml2-config ICU_LIBS=\"-L/opt/local/Current_v18/lib -licuuc -licudata -licui18n\" ICU_CFLAGS=\"-I/opt/local/Current_v18/include\" LZ4_CFLAGS=\"-I/opt/local/Current_v18/include\" LZ4_LIBS=\"-L/opt/local/Current_v18/lib\" ZSTD_CFLAGS=\"-I/opt/local/Current_v18/include\" ZSTD_LIBS=\"-L/opt/local/Current_v18/lib\" ./configure --with-icu --enable-debug  --prefix=$PG_STAGING --with-ldap --with-openssl --with-perl --with-python --with-tcl --with-bonjour --with-pam --enable-thread-safety --with-libxml --with-uuid=e2fs --with-includes=/opt/local/Current_v18/include/libxml2:/opt/local/Current_v18/include:/opt/local/Current_v18/include/security:/opt/local/Current_v18/include/openssl/ --docdir=$PG_STAGING/doc/postgresql --with-libxslt --with-libedit-preferred --with-gssapi --with-lz4 --with-zstd" || _die "Failed to configure postgres for universal support"
