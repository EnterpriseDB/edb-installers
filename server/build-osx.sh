...
_build_server_osx() {
    ...
    CFLAGS="$PG_ARCH_OSX_CFLAGS -arch i386" ./configure --host=powerpc-apple-darwin --prefix=$WD/server/staging/osx --with-openssl --with-perl --with-python --with-tcl --with-bonjour --with-pam --with-krb5  || _die "Failed to configure postgres for i386"
    mv src/include/pg_config.h src/include/pg_config_i386.h

    echo "Configuring the postgres source tree for PPC"
    CFLAGS="$PG_ARCH_OSX_CFLAGS -arch ppc" ./configure --host=i386-apple-darwin --prefix=$WD/server/staging/osx --with-openssl --with-perl --with-python --with-tcl --with-bonjour --with-pam --with-krb5 || _die "Failed to configure postgres for PPC"
