...
_prep_server_osx() {
    ...
    ssh $PG_SSH_OSX_PGADMIN "rm -f $PG_PATH_OSX_PGADMIN/pgadmin4/*.bz2" || _die "Failed to clean the server/*.bz2 files on Mac OSX pgAdmin4 build VM"
    ssh $PG_SSH_OSX_PGADMIN "rm -f $PG_PATH_OSX_PGADMIN/pgadmin4/*.sh" || _die "Falied to clean the server/*.sh scripts on Mac OSX pgAdmin4 build VM"
