...
_build_server_osx() {
    ...
    ssh $PG_SSH_OSX "cp /opt/local/Current_v15/certs/cacert.pem $PG_PATH_OSX/server/source/stackbuilder.osx/stackbuilder.app/Contents/Resources/certs/ca-bundle.crt" || _die "Failed to copy certs bundle"
    ...
    ssh $PG_SSH_OSX "cp -r /opt/local/Current_v15/include/openssl $PG_STAGING/include" || _die "Failed to copy the required header"
    ssh $PG_SSH_OSX "cp -r /opt/local/Current_v15/include/libxml2 $PG_STAGING/include" || _die "Failed to copy the required header"
    ssh $PG_SSH_OSX "cp -r /opt/local/Current_v15/include/libxslt $PG_STAGING/include" || _die "Failed to copy the required header"
    ssh $PG_SSH_OSX "cp -r /opt/local/Current_v15/include/unicode $PG_STAGING/include" || _die "Failed to copy the required header"
    ssh $PG_SSH_OSX "cp /opt/local/Current_v15/include/iconv.h $PG_STAGING/include" || _die "Failed to copy the required header"
    ssh $PG_SSH_OSX "cp /opt/local/Current_v15/include/zlib.h $PG_STAGING/include" || _die "Failed to copy the required header"
    ssh $PG_SSH_OSX "cp /opt/local/Current_v15/include/lz4*.h $PG_STAGING/include" || _die "Failed to copy the required header"
    ssh $PG_SSH_OSX "cp /opt/local/Current_v15/include/zdict.h $PG_STAGING/include" || _die "Failed to copy the required header"
    ssh $PG_SSH_OSX "cp /opt/local/Current_v15/include/zstd*.h $PG_STAGING/include" || _die "Failed to copy the required header"

    # Copy Lz4 libs to staging
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current_v15/lib/liblz4*.dylib $PG_STAGING/lib/" || _die "Failed to copy the LZ4 libs to staging"

    #cd $WD/server/staging/osx
    # Copy libxml2 as System's libxml can be old.
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current_v15/lib/libxml2*.dylib $PG_STAGING/lib/" || _die "Failed to copy the latest libxml2"
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current_v15/lib/libxslt*.dylib $PG_STAGING/lib/" || _die "Failed to copy the latest libxslt"
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current_v15/lib/libuuid*.dylib $PG_STAGING/lib/" || _die "Failed to copy the latest libuuid"
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current_v15/lib/libedit*.dylib $PG_STAGING/lib/" || _die "Failed to copy the latest libedit"
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current_v15/lib/libz*.dylib $PG_STAGING/lib/" || _die "Failed to copy the latest libz"
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current_v15/lib/libssl*.dylib $PG_STAGING/lib/" || _die "Failed to copy the latest libssl"
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current_v15/lib/libcrypto*.dylib $PG_STAGING/lib/" || _die "Failed to copy the latest libcrypto"
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current_v15/lib/libintl.*dylib $PG_STAGING/lib/" || _die "Failed to copy the latest libintl"
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current_v15/lib/libicui18n*dylib $PG_STAGING/lib/" || _die "Failed to copy the latest libicui18n"
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current_v15/lib/libicudata*dylib $PG_STAGING/lib/" || _die "Failed to copy the latest libicudata"
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current_v15/lib/libicuuc*dylib $PG_STAGING/lib/" || _die "Failed to copy the latest libicuuc"
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current_v15/lib/libiconv*.dylib $PG_STAGING/lib/" || _die "Failed to copy the latest libiconv"
    ...
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current_v15/lib/libwx_osx_cocoau_webview-*.dylib $PG_STAGING/stackbuilder.app/Contents/Frameworks/" || _die "Failed to copy the latest libwx"
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current_v15/lib/libwx_osx_cocoau_html-*.dylib $PG_STAGING/stackbuilder.app/Contents/Frameworks/" || _die "Failed to copy the latest libwx"
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current_v15/lib/libwx_osx_cocoau_qa-*.dylib $PG_STAGING/stackbuilder.app/Contents/Frameworks/" || _die "Failed to copy the latest libwx"
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current_v15/lib/libwx_osx_cocoau_adv-*.dylib $PG_STAGING/stackbuilder.app/Contents/Frameworks/" || _die "Failed to copy the latest libwx"
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current_v15/lib/libwx_osx_cocoau_core-*.dylib $PG_STAGING/stackbuilder.app/Contents/Frameworks/" || _die "Failed to copy the latest libwx"
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current_v15/lib/libwx_baseu_xml-*.dylib $PG_STAGING/stackbuilder.app/Contents/Frameworks/" || _die "Failed to copy the latest libwx"
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current_v15/lib/libwx_baseu_net-*.dylib $PG_STAGING/stackbuilder.app/Contents/Frameworks/" || _die "Failed to copy the latest libwx"
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current_v15/lib/libwx_baseu-*.dylib $PG_STAGING/stackbuilder.app/Contents/Frameworks/" || _die "Failed to copy the latest libwx"
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current_v15/lib/libcurl*dylib $PG_STAGING/stackbuilder.app/Contents/Frameworks/" || _die "Failed to copy the latest libcurl"
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current_v15/lib/libz*.dylib $PG_STAGING/stackbuilder.app/Contents/Frameworks/" || _die "Failed to copy the latest libz"
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current_v15/lib/libssl*.dylib $PG_STAGING/stackbuilder.app/Contents/Frameworks/" || _die "Failed to copy the latest libssl"
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current_v15/lib/libcrypto*.dylib $PG_STAGING/stackbuilder.app/Contents/Frameworks/" || _die "Failed to copy the latest libcrypto"
    ssh $PG_SSH_OSX "cp -pR /opt/local/Current_v15/lib/libjpeg*.dylib $PG_STAGING/stackbuilder.app/Contents/Frameworks/" || _die "Failed to copy the latest libjpeg"
