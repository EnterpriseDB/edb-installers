...
_postprocess_server_osx() {
    ...
    _replace "<requireInstallationByRootUser>0</requireInstallationByRootUser>" "<requireInstallationByRootUser>1</requireInstallationByRootUser>" installer_1.xml || _die "Failed to replace the requireInstallationByRootUser value to 1 in installer_1.xml"

    # Build the installer (for the root privileges required)
    echo Building the installer with the root privileges required
    "$PG_INSTALLBUILDER_BIN" build installer_1.xml osx || _die "Failed to build the installer"

    cp $WD/output/postgresql-$PG_MAJOR_VERSION-osx-installer.app/Contents/MacOS/PostgreSQL $WD/server/staging/osx/risePrivileges || _die "Failed to copy the privileges utility generated by the bitrock installer, when requireInstallationByRootUser set to 1."

    echo "Removing the installer previously generated installer"
    rm -rf $WD/output/postgresql-$PG_MAJOR_VERSION-osx-installer.app || _die "Failed to remove the first installer ($WD/output/postgresql-$PG_MAJOR_VERSION-osx-installer.app)"
    ...
    mv $WD/server/staging/osx/risePrivileges $WD/output/postgresql-$PG_MAJOR_VERSION-osx-installer.app/Contents/MacOS/PostgreSQL || _die "Failed to move the privileges utility in the newly generated installer"

    # Using own script for extract-only mode
