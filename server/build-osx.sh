...
_build_server_osx() {
    ...
    CFLAGS="$PG_ARCH_OSX_CFLAGS -arch ppc" LDFLAGS="-L/usr/local/lib" ./configure --host=powerpc-apple-darwin --prefix=$WD/server/staging/osx --with-openssl --with-perl --with-python --with-tcl --with-bonjour --with-pam --with-krb5 --enable-thread-safety --with-libxml --with-ossp-uuid --with-includes=/usr/local/include/libxml2:/usr/local/include --docdir=$WD/server/staging/osx/doc/postgresql || _die "Failed to configure postgres for PPC"
    mv src/include/pg_config.h src/include/pg_config_ppc.h

    echo "Configuring the postgres source tree for Universal"
    CFLAGS="$PG_ARCH_OSX_CFLAGS -arch ppc -arch i386" LDFLAGS="-L/usr/local/lib" ./configure --prefix=$WD/server/staging/osx --with-openssl --with-perl --with-python --with-tcl --with-bonjour --with-pam --with-krb5 --enable-thread-safety --with-libxml --with-ossp-uuid --with-includes=/usr/local/include/libxml2:/usr/local/include --docdir=$WD/server/staging/osx/doc/postgresql || _die "Failed to configure postgres for Universal"
    ...
    cd ../uuid-ossp
    make || _die "Failed to build the uuid-ossp module"
    ...
    if [ -f Authorization ]; then
        rm -f Authorization Authorization.o
    fi
    gcc -x c -arch i386 -arch ppc -fmessage-length=0 -pipe -Wno-trigraphs -fpascal-strings -fasm-blocks -Os -mdynamic-no-pic -Wmissing-prototypes $PG_ARCH_OSX_CFLAGS -fvisibility=hidden -gdwarf-2 -Wall -c Authorization.c -o Authorization.o || _die "Couldn't compile the custom Authorization tool for osx"
    gcc -arch i386 -arch ppc $PG_ARCH_OSX_CFLAGS -framework Carbon -framework Security -prebind -o Authorization Authorization.o || _die "Couldn't build the custom Authorization tool for osx"

    if [ -f InstallerHelper ]; then
        rm -f InstallerHelper InstallerHelper.o
    fi
    gcc -arch i386 -arch ppc $PG_ARCH_OSX_CFLAGS -c InstallerHelper.c -o InstallerHelper.o || _die "Couldn't compile the InstallerHelper tool for osx"
    gcc -arch i386 -arch ppc $PG_ARCH_OSX_CFLAGS -framework Carbon -framework Security -prebind -o InstallerHelper InstallerHelper.o || _die "Couldn't build the InstallerHelper tool for osx"
...
_postprocess_server_osx() {
    ...
    echo "Building the installer for OSX"
    ...
    cp -f $WD/scripts/osx/InstallerHelper $WD/output/postgresql-$PG_MAJOR_VERSION-osx-installer.app/Contents/MacOS/InstallerHelper
    chmod a+x $WD/output/postgresql-$PG_MAJOR_VERSION-osx-installer.app/Contents/MacOS/InstallerHelper
    cp -f $WD/scripts/osx/Authorization $WD/output/postgresql-$PG_MAJOR_VERSION-osx-installer.app/Contents/MacOS/PostgreSQL
