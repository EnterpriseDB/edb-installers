...
_build_server_linux() {
    ...
    ssh $PG_SSH_LINUX "cd $PG_PATH_LINUX/server/source/postgres.linux/; PYTHON=/opt/ActivePython-3.2/bin/python3.2 TCLSH=/opt/ActiveTcl-8.5/bin/tclsh TCL_CONFIG_SH=/opt/ActiveTcl-8.5/lib/tclConfig.sh PERL=/opt/ActivePerl-5.14/bin/perl ./configure --with-libs=/usr/local/openssl/lib:/usr/local/ldap-2.4.23/lib:/usr/local/lib --with-includes=/usr/local/openssl/include:/usr/local/ldap-2.4.23/include:/usr/local/include --prefix=$PG_STAGING --with-ldap --with-openssl --with-perl --with-python --with-tcl --with-tclconfig=$PG_TCL_LINUX/lib --with-pam --with-krb5 --enable-thread-safety --with-libxml --with-ossp-uuid --docdir=$PG_STAGING/doc/postgresql --with-libxslt --with-libedit-preferred --with-gssapi LD_LIBRARY_PATH=/usr/local/openssl/lib:/usr/local/ldap-2.4.23/lib:/usr/local/lib"  || _die "Failed to configure postgres"
    ...
    ssh $PG_SSH_LINUX "cd $PG_PATH_LINUX/server/source/postgres.linux/contrib; export LD_LIBRARY_PATH=/usr/local/openssl/lib:/usr/local/ldap-2.4.23/lib:/usr/local/lib:\$LD_LIBRARY_PATH; make -j4" || _die "Failed to build the postgres contrib modules"
    ...
    ssh $PG_SSH_LINUX "cp -r /usr/local/openssl/include $PG_STAGING/include" || _die "Failed to copy the required header"
    ...
    ssh $PG_SSH_LINUX "cp -r /usr/local/cyrus-sasl/include/sasl $PG_STAGING/include" || _die "Failed to copy the required header"
    ...
    ssh $PG_SSH_LINUX "cp /usr/local/ldap-2.4.23/include/ldap*.h $PG_STAGING/include" || _die "Failed to copy the required header"
    ssh $PG_SSH_LINUX "cp /usr/local/ldap-2.4.23/include/lber*.h $PG_STAGING/include" || _die "Failed to copy the required header"
    ...
    ssh $PG_SSH_LINUX "cp -R /usr/local/curl/lib/libcurl.so* $PG_STAGING/lib" || _die "Failed to copy the dependency library (libcurl)"
...
EOT
    ...
    ssh $PG_SSH_LINUX "cp -R /usr/lib/libjpeg.so* $PG_STAGING/pgAdmin3/lib" || _die "Failed to copy the dependency library"
...
#    cp "$WD/server/source/pljava.linux/docs/"* "$WD/server/staging/linux/doc/pljava/" || _die "Failed to install the pl/java documentation"
	...
	ssh $PG_SSH_LINUX "cd $PG_PATH_LINUX/server/source/stackbuilder.linux/; cmake -D CMAKE_BUILD_TYPE:STRING=Release -D CURL_ROOT:PATH=/usr/local/curl -D WX_CONFIG_PATH:FILEPATH=/usr/local/bin/wx-config -D WX_DEBUG:BOOL=OFF -D WX_STATIC:BOOL=ON -D CMAKE_INSTALL_PREFIX:PATH=$PG_STAGING/stackbuilder ."
...
#    cp resources/xdg/pg-doc-pljava.desktop staging/linux/scripts/xdg/pg-doc-pljava-$PG_VERSION_STR.desktop || _die "Failed to copy a menu pick"
  ...
  ssh $PG_SSH_LINUX "cd /buildfarm/PG91/src/test;  rm -rf regress; "  

  # Copy the regress folder into Regression Setup folder /buildfarm/PG91/
  ssh $PG_SSH_LINUX "cp -rf $PG_PATH_LINUX/server/source/postgres.linux/src/test/regress /buildfarm/PG91/src/test/;" 
