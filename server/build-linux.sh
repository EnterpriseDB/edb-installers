...
_build_server_linux() {
    ...
    ssh $PG_SSH_LINUX "cd $PG_PATH_LINUX/server/source/postgres.linux/; PYTHON=$PG_PYTHON_LINUX/bin/python3.2 TCLSH=$PG_TCL_LINUX/bin/tclsh TCL_CONFIG_SH=$PG_TCL_LINUX/lib/tclConfig.sh PERL=$PG_PERL_LINUX/bin/perl ./configure --enable-debug --with-libs=/usr/local/lib --with-includes=/usr/local/include:/usr/local/include/libxml2 --prefix=$PG_STAGING --with-ldap --with-openssl --with-perl --with-python --with-tcl --with-tclconfig=$PG_TCL_LINUX/lib --with-pam --with-krb5 --enable-thread-safety --with-libxml --with-ossp-uuid --docdir=$PG_STAGING/doc/postgresql --with-libxslt --with-libedit-preferred --with-gssapi LD_LIBRARY_PATH=/usr/local/lib"  || _die "Failed to configure postgres"

    echo "Building postgres"
    ssh $PG_SSH_LINUX "cd $PG_PATH_LINUX/server/source/postgres.linux; LD_LIBRARY_PATH=/usr/local/lib  make -j4 shared_libpython=yes" || _die "Failed to build postgres" 
    ssh $PG_SSH_LINUX "cd $PG_PATH_LINUX/server/source/postgres.linux; LD_LIBRARY_PATH=/usr/local/lib make shared_libpython=yes install" || _die "Failed to install postgres"
    ...
    ssh $PG_SSH_LINUX "cp -r /usr/local/include/openssl $PG_STAGING/include" || _die "Failed to copy the required header"
    ssh $PG_SSH_LINUX "cp -r /usr/local/include/libxml2 $PG_STAGING/include" || _die "Failed to copy the required header"
    ssh $PG_SSH_LINUX "cp -r /usr/local/include/libxslt $PG_STAGING/include" || _die "Failed to copy the required header"
    ssh $PG_SSH_LINUX "cp -r /usr/local/include/sasl $PG_STAGING/include" || _die "Failed to copy the required header"
    ssh $PG_SSH_LINUX "cp -r /usr/local/include/krb5 $PG_STAGING/include" || _die "Failed to copy the required header"
    ssh $PG_SSH_LINUX "cp -r /usr/local/include/gssapi $PG_STAGING/include" || _die "Failed to copy the required header"
    ssh $PG_SSH_LINUX "cp /usr/local/include/iconv.h $PG_STAGING/include" || _die "Failed to copy the required header"
    ssh $PG_SSH_LINUX "cp /usr/local/include/uuid.h $PG_STAGING/include" || _die "Failed to copy the required header"
    ssh $PG_SSH_LINUX "cp /usr/local/include/zlib.h $PG_STAGING/include" || _die "Failed to copy the required header"
    ssh $PG_SSH_LINUX "cp /usr/local/include/krb5.h $PG_STAGING/include" || _die "Failed to copy the required header"
    ssh $PG_SSH_LINUX "cp /usr/local/include/ncurses*.h $PG_STAGING/include" || _die "Failed to copy the required header"
    ssh $PG_SSH_LINUX "cp /usr/local/include/gssapi.h $PG_STAGING/include" || _die "Failed to copy the required header"
    ssh $PG_SSH_LINUX "cp /usr/local/include/ldap*.h $PG_STAGING/include" || _die "Failed to copy the required header"
    ...
    ssh $PG_SSH_LINUX "cp -pR /usr/local/lib/libcurl.so* $PG_STAGING/lib" || _die "Failed to copy the dependency library(libcurl)"
...
EOT
    ...
    ssh $PG_SSH_LINUX "cd $PG_PATH_LINUX/server/source/stackbuilder.linux/; cmake -D CMAKE_BUILD_TYPE:STRING=Release -D CURL_ROOT:PATH=/usr/local -D WX_CONFIG_PATH:FILEPATH=/usr/local/bin/wx-config -D WX_DEBUG:BOOL=OFF -D WX_STATIC:BOOL=ON -D CMAKE_INSTALL_PREFIX:PATH=$PG_STAGING/stackbuilder ."
   ...
   ssh $PG_SSH_LINUX "cp /usr/local/certs/ca-bundle.crt $PG_STAGING/stackbuilder/share/certs" || _die "Failed to copy the certs directory"
