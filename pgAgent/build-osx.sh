...
_prep_pgAgent_osx() {
    ...
    cp -R pgagent-pgagent-$PG_VERSION_PGAGENT/* pgAgent.osx || _die "Failed to copy the source code (source/pgAgent-$PG_VERSION_PGAGENT)"
...
cat <<EOT-PGAGENT > $WD/pgAgent/build-pgagent.sh
    ...
    cd \$SOURCE_DIR
    BOOST_ROOT=/opt/local/Current_v18/boost PGDIR=$PG_PGHOME_OSX $PG_CMAKE_OSX -DCMAKE_INSTALL_PREFIX=\$PG_STAGING/pgAgent -DCMAKE_OSX_ARCHITECTURES:STRING="x86_64;arm64" -DCMAKE_OSX_DEPLOYMENT_TARGET:STRING=$MACOSX_MIN_VERSION -DBOOST_LIBRARYDIR:PATH=/opt/local/Current_v18/boost/lib -DSTATIC_BUILD=NO -D CMAKE_OSX_SYSROOT:FILEPATH=$SDK_PATH -DCMAKE_CXX_STANDARD=17 CMakeLists.txt || _die "Couldn't configure the pgAgent sources"
    ...
    cp -pR /opt/local/Current_v18/boost/lib/libboost_filesystem.dylib \$PG_STAGING/pgAgent/lib || _die "Failed to copy required dylibs to staging directory"
    cp -pR /opt/local/Current_v18/boost/lib/libboost_system.dylib \$PG_STAGING/pgAgent/lib || _die "Failed to copy required dylibs to staging directory"
    cp -pR /opt/local/Current_v18/boost/lib/libboost_thread.dylib \$PG_STAGING/pgAgent/lib || _die "Failed to copy required dylibs to staging directory"
    cp -pR /opt/local/Current_v18/boost/lib/libboost_chrono.dylib \$PG_STAGING/pgAgent/lib || _die "Failed to copy required dylibs to staging directory"
    cp -pR /opt/local/Current_v18/boost/lib/libboost_regex.dylib \$PG_STAGING/pgAgent/lib || _die "Failed to copy required dylibs to staging directory"
    cp -pR /opt/local/Current_v18/boost/lib/libboost_date_time.dylib \$PG_STAGING/pgAgent/lib || _die "Failed to copy required dylibs to staging directory"
    cp -pR /opt/local/Current_v18/boost/lib/libboost_atomic.dylib \$PG_STAGING/pgAgent/lib || _die "Failed to copy required dylibs to staging directory"
    ...
    install_name_tool -change "@rpath/libboost_system.dylib" "@loader_path/../lib/libboost_system.dylib" "\$PG_STAGING/pgAgent/bin/pgagent"
    install_name_tool -change "@rpath/libboost_chrono.dylib" "@loader_path/../lib/libboost_chrono.dylib" "\$PG_STAGING/pgAgent/bin/pgagent"
    install_name_tool -change "@rpath/libboost_atomic.dylib" "@loader_path/../lib/libboost_atomic.dylib" "\$PG_STAGING/pgAgent/bin/pgagent"
...
_postprocess_pgAgent_osx() {
   ...
   echo ssh $PG_SSH_OSX_NOTARY "cd $PG_PATH_OSX_NOTARY; KEYCHAIN_PASSWD="$KEYCHAIN_PASSWD" ./notarize_apps.sh pgagent-pg$PG_CURRENT_VERSION-$PG_VERSION_PGAGENT-$PG_BUILDNUM_PGAGENT-${BUILD_FAILED}osx.zip pgagent" || _die "Failed to notarize the app"
   ssh $PG_SSH_OSX_NOTARY "cd $PG_PATH_OSX_NOTARY; KEYCHAIN_PASSWD="$KEYCHAIN_PASSWD" ./notarize_apps.sh pgagent-pg$PG_CURRENT_VERSION-$PG_VERSION_PGAGENT-$PG_BUILDNUM_PGAGENT-${BUILD_FAILED}osx.zip pgagent" || _die "Failed to notarize the app"
