...
cat <<EOT-PGAGENT > $WD/pgAgent/build-pgagent.sh
    ...
    cd \$SOURCE_DIR
    BOOST_ROOT=/opt/local/Current_v15/boost PGDIR=$PG_PGHOME_OSX $PG_CMAKE_OSX -DCMAKE_INSTALL_PREFIX=\$PG_STAGING/pgAgent -DCMAKE_OSX_ARCHITECTURES:STRING="x86_64;arm64" -DCMAKE_OSX_DEPLOYMENT_TARGET:STRING=$MACOSX_MIN_VERSION -DSTATIC_BUILD=NO -D CMAKE_OSX_SYSROOT:FILEPATH=$SDK_PATH CMakeLists.txt || _die "Couldn't configure the pgAgent sources"
    ...
    cp -pR /opt/local/Current_v15/boost/lib/libboost_filesystem.dylib \$PG_STAGING/pgAgent/lib || _die "Failed to copy required dylibs to staging directory"
    cp -pR /opt/local/Current_v15/boost/lib/libboost_system.dylib \$PG_STAGING/pgAgent/lib || _die "Failed to copy required dylibs to staging directory"
    cp -pR /opt/local/Current_v15/boost/lib/libboost_thread.dylib \$PG_STAGING/pgAgent/lib || _die "Failed to copy required dylibs to staging directory"
    cp -pR /opt/local/Current_v15/boost/lib/libboost_chrono.dylib \$PG_STAGING/pgAgent/lib || _die "Failed to copy required dylibs to staging directory"
    cp -pR /opt/local/Current_v15/boost/lib/libboost_regex.dylib \$PG_STAGING/pgAgent/lib || _die "Failed to copy required dylibs to staging directory"
    cp -pR /opt/local/Current_v15/boost/lib/libboost_date_time.dylib \$PG_STAGING/pgAgent/lib || _die "Failed to copy required dylibs to staging directory"
    cp -pR /opt/local/Current_v15/boost/lib/libboost_atomic.dylib \$PG_STAGING/pgAgent/lib || _die "Failed to copy required dylibs to staging directory"
