...
_build_pgAgent_linux() {
    ...
    ssh $PG_SSH_LINUX "cd $SOURCE_DIR; LDFLAGS=' -L$PG_PGHOME_LINUX/lib -lldap ' LD_LIBRARY_PATH=$PG_PGHOME_LINUX/lib PGDIR=$PG_PGHOME_LINUX cmake -DCMAKE_INSTALL_PREFIX=$PG_STAGING CMakeLists.txt" || _die "Couldn't configure the pgAgent sources"
    ...
    ssh $PG_SSH_LINUX "cp -R $PG_PGHOME_LINUX/lib/libsasl2.so* $PG_STAGING/lib" || _die "Failed to copy the dependency library (libsasl2)"
