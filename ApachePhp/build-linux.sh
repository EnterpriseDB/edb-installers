...
_build_ApachePhp_linux() {
    ...
    echo "Building php"
    ssh $PG_SSH_LINUX "cd $PG_PATH_LINUX/ApachePhp/source/php.linux; export LD_LIBRARY_PATH=$PG_PGHOME_LINUX/lib; make" || _die "Failed to build php"
    ...
    # Copy in the dependency libraries (php)
    ...
    ssh $PG_SSH_LINUX "cp -R /usr/lib/libjpeg.so* $PG_STAGING/php/lib" || _die "Failed to copy the dependency library(libjpeg)"
    ssh $PG_SSH_LINUX "cp -R /usr/lib/libfreetype.so* $PG_STAGING/php/lib" || _die "Failed to copy the dependency library(libfreetype)"
    ...
    ssh $PG_SSH_LINUX "cd $PG_STAGING/php/bin; chrpath --replace \\\${ORIGIN}/../lib:\\\${ORIGIN}/../../apache/lib php"

    # Change the rpath for php
    ssh $PG_SSH_LINUX "cd $PG_STAGING/apache; chrpath --replace \\\${ORIGIN}/../lib:\\\${ORIGIN}/../../php/lib modules/libphp5.so"
    ssh $PG_SSH_LINUX "cd $PG_STAGING/apache; FILES=\`file \\\`find . -maxdepth 2 -mindepth 2\\\` | grep ELF | cut -d: -f1\`; for F in \$FILES; do RPATH=\`chrpath \$F | grep RPATH | grep -v ORIGIN\`; if [[ x\"\${RPATH}\" != x\"\" ]]; then chrpath --replace \\\${ORIGIN}/../lib \$F; fi done"
