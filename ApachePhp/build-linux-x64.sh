...
EOT
    ...
    if [ -e $WD/output/symbols/linux-x64/ApachePhp ];
    then
        echo "Removing existing $WD/output/symbols/linux-x64/ApachePhp directory"
        rm -rf $WD/output/symbols/linux-x64/ApachePhp  || _die "Couldn't remove the existing $WD/output/symbols/linux-x64/ApachePhp directory."
    fi
