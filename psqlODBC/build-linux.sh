...
_build_psqlODBC_linux() {
    ...
    ssh $PG_SSH_LINUX "cp -R /usr/lib/libodbc.so* $PG_STAGING/lib" || _die "Failed to copy the dependency library"
    ...
    ssh $PG_SSH_LINUX "cp -R $PG_PGHOME_LINUX/lib/libcrypto.so* $PG_STAGING/lib" || _die "Failed to copy the dependency library (libcrypt)"
