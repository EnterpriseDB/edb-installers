...
"$INSTALLDIR/Drupal7/installer/xdg/xdg-icon-resource" install --size 32 "$INSTALLDIR/Drupal7/scripts/images/pg-launchDrupal7.png"

# Fixup the scripts
_fixup_file "$INSTALLDIR/Drupal7/scripts/launchbrowser.sh"
_fixup_file "$INSTALLDIR/Drupal7/scripts/launchDrupal7.sh"

chmod ugo+x "$INSTALLDIR/Drupal7/scripts/"*.sh
chmod ugo+x "$INSTALLDIR/Drupal7/installer/Drupal7/"*.sh

# Fixup the XDG files (don't just loop in case we have old entries we no longer want)
_fixup_file "$INSTALLDIR/Drupal7/scripts/xdg/pg-postgresql.directory"
_fixup_file "$INSTALLDIR/Drupal7/scripts/xdg/pg-launchDrupal7.desktop"
...
"$INSTALLDIR/Drupal7/installer/xdg/xdg-desktop-menu" install --mode system  \
      ...
      "$INSTALLDIR/Drupal7/scripts/xdg/pg-launchDrupal7.desktop"  || _warn "Failed to create the Drupal7 menu"
