...
_build_pgbouncer_linux() {
    ...
    ssh $PG_SSH_LINUX "cp -R /usr/lib/libreadline.so* $PG_PATH_LINUX/pgbouncer/staging/linux/instscripts/" || _die "Failed to copy the dependency library"
...
_postprocess_pgbouncer_linux() {
    ...
    _replace "listen_addr = 127.0.0.1" "listen_addr = @@LISTENADDR@@" staging/linux/pgbouncer/share/pgbouncer.ini || _die "Failed to put the place holder"
