...
EOT
    ...
    ssh $PG_SSH_WINDOWS "cmd /c copy C:\\\\pgBuild\\\\openssl-ppas\\\\bin\\\\ssleay32.dll $PG_PATH_WINDOWS\\\\pgbouncer.staging\\\\instscripts" || _die "Failed to copy a dependency DLL on the windows build host"
    ssh $PG_SSH_WINDOWS "cmd /c copy C:\\\\pgBuild\\\\openssl-ppas\\\\bin\\\\libeay32.dll $PG_PATH_WINDOWS\\\\pgbouncer.staging\\\\instscripts" || _die "Failed to copy a dependency DLL on the windows build host"
